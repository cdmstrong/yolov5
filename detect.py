        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    images = imc[int(xyxy[1].item()): int(xyxy[3].item()), int(xyxy[0].item()): int(xyxy[2].item())]
                    images = cv2.resize(images, (94,24), interpolation=cv2.INTER_AREA)
                    # cv2.imshow('h', images)
                    # cv2.waitKey(0)
                    numDetect.detect(images.transpose(2, 0, 1))
            # Stream results
...
class Detect():
    def __init__(self) -> None:
        ...
        PATH = "./weights/Final_LPRNet_model.pth"
        self.lprnet.load_state_dict(torch.load(PATH,map_location=torch.device('cpu')))
